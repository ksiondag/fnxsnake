...
printString .macro address, length
    #ngn.load16BitImmediate \address, ngn.TXT_PTR3
    lda #\length
    jsr ngn.txtio.printStr
.endmacro
...
inputString .macro addrRes, lenRes, addrAllowed, lenAllowed
    #ngn.load16BitImmediate \addrRes, TXT_PTR4
    ldx #\lenRes
    #ngn.load16BitImmediate \addrAllowed, TXT_PTR3
    ldy #\lenAllowed
    jsr ngn.txtio.getString    
.endmacro
...
cursorState_t .struct 
    ...
    yMaxMinus1  .byte 59
    col         .byte $10         ; Default white foreground, black background
    tempIo      .byte 0
    nextChar    .byte 0
...
; address is stored in .videoRamPtr.
; --------------------------------------------------
calcCursorOffset
    ; calculate y * xMax    
...
getScreenPtr
    ...
    #move16Bit MUL_RES_CO_PROC, TXT_PTR1
...
_inputLoop
    ; TODO: This is broken, instead of loop this should be callback-ified
    ;jsr waitForKey
